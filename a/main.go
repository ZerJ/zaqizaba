...
func main() {
	...
	cipherHex, _, err := EncryptAESGCM([]byte(checksum), bytes, iv) // _0x3d5057['toHex']()
	token := fmt.Sprintf("zoey::%s::%s", ivBase64, cipherHex)
	fmt.Println(token)
}
...
func randomBytesToString(len int) ([]byte, error) {
	b := make([]byte, len)
	_, err := rand.Read(b)
	if err != nil {
		return nil, err
...
func EncryptAESGCM(plaintext, key, iv []byte) (cipherHex string, ivHex string, err error) {
